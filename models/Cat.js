class Cat {
    ...
    static async createCat (insertValues) {
        // const {rows} = await this._client.query(`INSERT INTO ${this._tableName} 
        // (name, breed, color, birthday, weight) VALUES ('${name}', '${breed}', '${color}', '${birthday}', ${weight}) 
        // RETURNING *;`);
        // return rows;
        ...
        const str = `INSERT INTO ${this._tableName} (${insertAttrSchema}) VALUES (${insertValueString}) 
        RETURNING *;`
    ...
    static async deleteCat (catId){
        const {rows: deletedCat} = await this._client.query(`DELETE FROM ${this._tableName} WHERE id = ${catId} 
        RETURNING *;`);
        return deletedCat;
    }
    ...
    static async updateCat ({catId, updateValues}){
        ...
        const {rows: updatedCat} = await this._client.query(`UPDATE ${this._tableName} SET ${insertValueString} 
        WHERE id = ${catId} RETURNING *;`);
        return updatedCat;
    }
